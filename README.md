...
- `search`:
  - получает список пакетов с `https://developer.auroraos.ru/conan`
  - фильтрует пакеты по подстроке из `<dependency>`
  - по каждому найденному пакету получает версии со страницы `https://developer.auroraos.ru/conan/<package>`
  - возвращает строки вида `<package>/<version>@aurora`
- `download`:
  - получает данные пакета со страницы `https://developer.auroraos.ru/conan/<dependency>`
  - выбирает указанную версию
  - скачивает все доступные архивы этой версии (обычно `armv7`, `armv8`, `x86_64`)
  - сохраняет в `./downloads/<dependency>/<version>/`
- `deps`:
  - получает зависимости пакета без использования `conan`
  - сначала берёт `conanInfo.requires` из `__NEXT_DATA__` для нужной версии
  - если страница пакета недоступна, пытается получить список версий из Artifactory storage API
  - если `conanInfo.requires` пуст, использует fallback-парсинг `self.requires(...)` из `conanfile`
